...
The fix reloads the =i2c_hid_acpi= kernel module after every resume, forcing
the touchpad to re-initialize cleanly. After the device reappears, it also
reapplies libinput settings (natural scrolling, tap-to-click, button areas) in
the active X session to avoid race conditions with user-space configuration.
...
| File                           | Description                                    |
|--------------------------------+------------------------------------------------|
| =touchpad-fix-resume.sh=      | Fix script (module reload + libinput settings)  |

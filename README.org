...
the touchpad to re-initialize cleanly. libinput settings (natural scrolling,
tap-to-click, click method) should be configured separately via
=/etc/X11/xorg.conf.d/= so they apply automatically to any new device.
...
| File                           | Description                                    |
|--------------------------------+------------------------------------------------|
| =touchpad-fix-resume.sh=      | Fix script (module reload)                      |
